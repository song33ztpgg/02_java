...
public class CalcTest {
	...
	public static void main(String[] args) {
		...
		int addResult = calc.add(input.getX(),input.getY());
		
		// TODO 4.2 int 타입의 변수 subResult 에 
		//          Calculator 클래스의 add 메소드 호출결과 저장
		int subResult = calc.subtract(input.getX(),input.getY());
		
		// TODO 4.3 int 타입의 변수 multResult 에 
		//          Calculator 클래스의 add 메소드 호출결과 저장
		int multResult = calc.multiply(input.getX(),input.getY());
		
		// TODO 4.4 int 타입의 변수 divResult 에 
		//          Calculator 클래스의 add 메소드 호출결과 저장
		int divResult = calc.divide(input.getX(),input.getY());
